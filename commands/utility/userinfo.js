...
const capitalizeFirstLetter = (string) => { return string.charAt(0).toUpperCase() + string.slice(1).toLowerCase(); };
...
module.exports = new Command({
    ...
    run: async ({ client, message, args }) => {
        ...
        // Get member data
        const user = message.guild.members.cache.get(message.mentions.members.first()?.id) || message.guild.members.cache.get(args[0]) || message.guild.members.cache.get(message.author.id);

        let userRoles = user.roles.cache.map(x => x).filter(z => z.name !== '@everyone');
        if (userRoles.length > 100) userRoles = `More than 100`

        let safe = user.user.createdTimestamp;
        if (safe < Date.now() - 2629746000) safe = `\`Not Suspicious\` <:online:925993330779779102>`;
        else safe = `\`Suspicious\` <:dnd:925993514536431676>`;
        ...
        for (i in totalPerms) if (PERM_FLAGS_MAP.hasOwnProperty(totalPerms[i])) keyPerms.push(PERM_FLAGS_MAP[totalPerms[i]]);
        keyPerms.sort();

        if (nitroBadge.includes('gif')) flags += `\n**•** <:boost:925990198360293497> \`Nitro\``;
...
**•** Account Created: \`${moment(user.user.createdAt).format("MMMM Do YYYY, H:mm:ss")}\`
**•** Joined Server: \`${moment(user.joinedAt).format("MMMM Do YYYY, H:mm:ss")}\`
**•** ID: \`${user.id}\`
**•** Tag: \`${user.user.tag}\`
